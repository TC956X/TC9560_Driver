...
void dwmac_mmc_ctrl(void __iomem *mmcaddr, unsigned int mode)
{
	...
	pr_debug("tc9562mac: MMC ctrl register (offset 0x%x): 0x%08x\n",
		 MMC_CNTRL, value);
}
...
{
	writel(0, mmcaddr + MMC_RX_INTR_MASK);
	writel(0, mmcaddr + MMC_TX_INTR_MASK);
	writel(MMC_DEFAULT_MASK, mmcaddr + MMC_RX_IPC_INTR_MASK);
	writel(MMC_FPE_TX_DEFAULT_MASK, mmcaddr + MMC_FPE_TX_INTR_MASK);
	writel(MMC_FPE_RX_DEFAULT_MASK, mmcaddr + MMC_FPE_RX_INTR_MASK);
}
 ...
 * Always call this function with mmc_lock spinlock 
 */
void dwmac_mmc_read(void __iomem *mmcaddr, struct tc9562mac_counters *mmc)
{
	...
	mmc->mmc_rx_pkt_assembly_err += readl(mmcaddr + MMC_RX_PKT_ASSEMBLY_ERR);
	mmc->mmc_rx_pkt_smd_err += readl(mmcaddr + MMC_RX_PKT_SMD_ERR);
	mmc->mmc_rx_pkt_assembly_ok += readl(mmcaddr + MMC_RX_PKT_ASSEMBLY_OK);
	mmc->mmc_rx_fpe_fragment += readl(mmcaddr + MMC_RX_FPE_FRAGMENT);
	mmc->mmc_tx_fpe_fragments += readl(mmcaddr + MMC_TX_FPE_FRAGMENT);
