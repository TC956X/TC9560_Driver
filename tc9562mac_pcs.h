...
#ifndef __TC9562MAC_PCS_H__
#define __TC9562MAC_PCS_H__
				 ...
				 struct tc9562mac_extra_stats *x)
{
	...
	if (intr_status & PCS_ANE_IRQ) {
		...
		if (val & GMAC_AN_STATUS_ANC)
			pr_info("tc9562mac_pcs: ANE process completed\n");
	}

	if (intr_status & PCS_LINK_IRQ) {
		x->irq_pcs_link_n++;
		if (val & GMAC_AN_STATUS_LS)
			pr_info("tc9562mac_pcs: Link Up\n");
		else
			pr_info("tc9562mac_pcs: Link Down\n");
	}
}
