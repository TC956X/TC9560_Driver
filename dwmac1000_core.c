...
static void dwmac1000_core_init(struct net_device *ndev, struct mac_device_info *hw, int mtu)
{
...
{
	void __iomem *ioaddr = hw->pcsr;
	tc9562mac_set_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
			    GMAC_ADDR_LOW(reg_n));
}
...
{
	void __iomem *ioaddr = hw->pcsr;
	tc9562mac_get_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
			    GMAC_ADDR_LOW(reg_n));
}
				 ...
				 int mcbitslog2)
{
	...
	case 8:
		...
		break;
	default:
		pr_debug("TC9562MAC: err in setting multicast filter\n");
		return;
		break;
	}
				 ...
				 struct net_device *dev, unsigned int l2_address_filtering)
{
	...
	if (netdev_uc_count(dev) > perfect_addr_number)
		...
		netdev_for_each_uc_addr(ha, dev) {
			tc9562mac_set_mac_addr(ioaddr, ha->addr,
					    GMAC_ADDR_HIGH(reg),
					    GMAC_ADDR_LOW(reg));
			reg++;
		}
...
static void dwmac1000_rgsmii(void __iomem *ioaddr, struct tc9562mac_extra_stats *x)
{
	u32 status;
...
static void dwmac1000_debug(void __iomem *ioaddr, struct tc9562mac_extra_stats *x,
...
static const struct tc9562mac_ops dwmac1000_ops = {
